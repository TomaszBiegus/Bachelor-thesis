...
%\item zbiór otwarty (korzystamy z pojecia kuli otwartej) jest
%\item pokazac ze kula otwarta jest zbiorem otwartym jest
%\item otoczenie punktu jest
%\item sąsiedztwo punktu jest
%\item wnętrze zbioru jest 
%\item domknięcie jest
%\item zbiór domknięty w przestrzeni metrycznej, jest
%\item zbiór brzegowy, jest
\item kula domknięta
...
%\item niezmienniczosc zbioru ze wzgledu na odwzorowanie, jest


%\item entropia topologiczna Adler, jest
%\item entropia topologiczna Bowen, jest
...
\textcolor{red}{TODO: granica górna granica dolna - występuje przy entropii Bowena}
...
\begin{proof}
\textcolor{red}{TODO: jak zabraknie czasu to tylko zacytowac artykul}
\end{proof}
...
Oznaczmy teraz $f(x) = \lim_{n \rightarrow \infty} f_n(x).$ 


Pozostało pokazać, że ciąg $(f_n)_{n=0}^{\infty} \subset C(X)$ zbiega do $f$ w przestrzeni $(C(X), d_1).$ Weźmy dowolny $\epsilon > 0.$ Pokażemy, że istnieje $n_0 \in \mathbb{N},$ takie że dla każdego $n > n_0$ zachodzi

$$d_1(f_n, f) = \max_{x \in X} \rho\big(f_n(x), f(x)\big) < \epsilon$$

Z definicji odwzorowania $f$, dla każdego $x \in X$ ciąg $\big(f_n(x)\big)_{n=0}^{\infty}$ jest zbieżny do $f(x)$, a więc istnieje $n_x \in \mathbb{N},$ takie że $\forall_{n > n_x} \, \rho\big(f_n(x), f(x)\big) < \epsilon.$ Wystarczy więc wziąć $n_0 = \max_{x \in X} n_x.$
...
Wówczas przestrzeń metryczna $(C_\triangle(X \times Y), d_2)$ jest zupełna i topologia na niej jest topologią zbieżności jednostajnej.
\end{lemma}

\begin{proof}
{\color{red} !!!NOWE!!!
{\color{green} Treść notatki od Pana (później oczywiście to usunę)
Tutaj mój pomysł na strategię dowodu byłby takim, żeby pokazać, że odwzorowania trójkątne tworzą zbiór domknięty w przestrzeni wszystkich ciągłych odwzorowań produktu w siebie (z metryką zbieżności jednostajnej). Ta ostatnia jest zupełna, co pokazaliśmy wyżej, a podprzestrzeń domknięta przestrzeni zupełnej też jest zupełna.}

Rozważmy przestrzeń $(C(X \times Y), d)$ wszystkich ciągłych odwzorowań postaci: 
$$F: X \times Y \rightarrow X \times Y; \, F(x,y) = \big(f_x(x,y), f_y(x,y)\big)$$ 
z metryką 
$$d(F, G) = \max_{(x,y) \in X \times Y} \max \left\{ \rho\big(f_x(x,y), g_x(x,y)\big), \tau\big(f_y(x,y), g_y(x,y)\big) \right\}$$ 
{\color{blue}(pytanie - czy wystarczy nam max a nie sup?, wynika to ze zwartości przestrzeni X i Y?)}

Pokażemy, że zbiór $(C_\triangle(X \times Y), d_2)$ jest domkniętym podzbiorem $(C(X \times Y), d)$ {\color{blue}(Pytanie: pokazać, że $d$ obcięte do $C_\triangle(X \times Y)$ jest równe $d_2$??)}
Weźmy dowolny zbieżny ciąg odwzorowań: $(F_n)_{n=1}^{\infty} \subset C_\triangle(X \times Y).$ Oznaczmy $F = \lim_{n \rightarrow \infty} F_n.$
Mamy 
$$\forall_{\epsilon > 0} \, \exists_{n_0} \, \forall_{n > n_0} d(F_n, F) = \max_{(x,y) \in X \times Y} \max \left\{ \rho\big(f_{n,x}(x,y), f_x(x,y)\big), \tau\big(f_{n,y}(x,y), f_y(x,y)\big) \right\} < \epsilon,$$
oznacza to, że 
$$\forall_{(x,y) \in X \times Y} \, \lim_{n \rightarrow \infty} \rho\big(f_{n,x}(x,y), f_x(x,y)\big) = \lim_{n \rightarrow \infty} \tau\big(f_{n,y}(x,y), f_y(x,y)\big) = 0.$$
Ponieważ $\forall_{n \in \mathbb{N}} \, F_n \in C_\triangle(X \times Y),$ to $f_{n,x}(x,y) = f_{n,x}(x).$ 
Mamy więc 
$$\forall_{(x,y) \in X \times Y} \, \lim_{n \rightarrow \infty} \rho\big(f_{n,x}(x), f_x(x,y)\big) = 0,$$
czyli odwzorowania $f_x$ nie mogą zależeć od współrzędnej $y.$ Zatem odwzorowanie $F$ jest odwzorowaniem trójkątnym, czyli zbiór odwzorowań trójkątnych jest domkniętym podzbiorem przestrzeni $C(X \times Y).$ 
...
{\color{blue} Tutaj nie jestem pewien czy lemat \ref{przestrzen_ciaglych_jest_zupelna} można tak bezpośrednio przenieść do sytuacji na produkcie, mniej więcej to widzę i wiem, że tak jest ale czy dostatecznie jest to w tekście wykazane? Ponadto tak samo jak w poprzednim lemacie - zostawić część zdania "... i topologia na niej jest topologią zbieżności jednostajnej" czy to usinąć? Jeżeli zostawić to chyba powinienem w sekcji z definicjami dorzucić co to znaczy topologia zbieżności jednostajnej}
...
\begin{proof}
{\color{red} !!!NOWE!!! {\color{blue}Pytanie: czy zawsze zlozenie funkcji ciaglych jest ciagle? Korzystam tutaj z tego. Poza tym wygląda na to że wystarczyły jedynie założenia o ciągłości f i skończoności niezmienniczego zbioru P, czy ja czegoś nie przeoczyłem?}
W pierwszej kolejności pokażemy prawdziwość tezy dla $N=2.$
