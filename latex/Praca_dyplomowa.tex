...
Niech $(X, \rho)$ będzie zwartą przestrzenią metryczną, a $f: X \rightarrow X$ ciągłym odwzorowaniem {\color{red}(TODO: ma byc uniformly continous - tak jest u Bowena, jest tak ze zwartości $X$, napisać to tutaj?)}.
...
{\color{red} !!!NOWE!! {\color{blue} Definicję entropii bowena wzialem z pracy Ruette, ona tam nie rozwazala entropii na podzbiorze tak jak Bowen, dlatego mialem klopot z interpretacja oznaczenia h(F, A) gdzie A jest zbiorem, w celu unikniecia przepisywania na nowo definicji zamiescilem tutaj tylko taka notke. Moze dopisac ze ruette defniuje nieco inaczej a w oryginale bowen korzysta z takich entropii jak ponizej i pozniej wlasciwa entropie na calej przestrzeni dostaje bierzac supremum po wszystkich zwartych podzbiorach przestrzeni?}
...
Rozważamy $X \times I$, oraz $X.$ Są to zwarte przestrzenie metryczne {\color{red}(TODO: pokazać zwartość? Jaką metrykę mamy na $X \times I$? max($\rho_x, \rho_y$)?)}.
...
{\color{orange}
Mówimy, że zbiór $E \subset X$ jest $(n, \epsilon)$-rozdzielony jeżeli dla każdych $x, y \in E,$ takich że $x \neq y$ istnieje $k \in \{0, 1, \ldots, n-1\},$ takie że $\rho(f^k(x), f^k(y)) > \epsilon.$
Maksymalną moc zbioru $(n, \epsilon)$-rozdzielonego zawartego w $K$ oznaczamy przez $s_n(f, \epsilon, K).$
$h_{\textrm{top}}(f, K) = \lim_{\epsilon \rightarrow 0} \limsup_{n \rightarrow +\infty} \frac{1}{n} \log s_n(f, \epsilon, K).$}
...
{\color{red} !!!NOWE!!! (Pokazuję ze $h(F) \geq h(f)$)
...
{\color{red}
{\color{blue}Poniżej alternatywa dla powyższego dowodu, jeżeli powyższy jest dobry, to tego alternatywnego nie będę wykorzystywał.}
\begin{proof}
Alternatywa (dowód alternatywny).
