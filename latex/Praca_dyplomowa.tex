...
Ustalmy dowolne $\epsilon > 0.$ Załóżmy, że nie możemy pokryć $X$ skończenie wieloma kulami o promieniu $\epsilon.$ Skonstruujemy wówczas ciąg $(K_i(s_i, \epsilon))_{i=0}^{\infty}$ rozłącznych kul zawartych w $X.$

Możemy skonstruować taki ciąg kul, ponieważ gdybyśmy w którymś kroku nie mogli dołożyć kolejnej kuli rozłącznej ze wszystkimi poprzednimi, znaczyłoby to, że nie istnieje punkt odległy od środków wszystkich poprzednich kul o więcej niż $2 \epsilon.$ Znaczyłoby to, że każdy punkt przestrzeni znajdowałby się bliżej niż $2 \epsilon$ od środka którejś ze skończenie wielu kul. W takim razie, aby pokryć całą przestrzeń należałoby pokryć skończenie wiele kul o promieniach $2 \epsilon$ kulami o promieniu $\epsilon,$ a do tego wystarczyłoby skończenie wiele kul o promieniu $\epsilon.$

Ciąg środków tych kul oczywiście nie zawiera podciągu zbieżnego gdyż dla każdych $i,j \in \mathbb{N}, $ $i \neq j: $ $\rho(s_i, s_j) \geq 2 \epsilon.$ Co prowadzi do sprzeczności, więc przestrzeń $X$ jest całkowicie ograniczona.
