...
Niech $(X, \rho)$ będzie przestrzenią metryczną, a $f: X \rightarrow X$ ciągłym odwzorowaniem.
...
Niech $I = [0, 1]$ i niech $f: I \rightarrow I$ będzie funkcją monotoniczną oraz taką, że $f(0) = 0$ i $f(1) = 1.$
...
Weźmy dowolny odcinek $B_j = [b_{j-1}, b_{j}] \, j \in \{1, 2, \ldots, |B|-1\}.$ Wybierzmy z niego dwa dowolne punkty $x, y$, $x \neq y$. Bez straty ogólności, niech $x > y.$ Oczywiście nie istnieje punkt $b_k \in B$ dla którego $y \leq b_k \leq x.$ Z konstrukcji $B$ widzimy natychmiast, że $\forall_{i \in \{0, 1, \ldots, n-1\}} \, \exists_{j \in \{1,2,\ldots,m\}} \, f^{-i}(a_{j-1}) \leq y < x \leq f^{-i}(a_j)$. Z monotoniczności funkcji $f$ otrzymujemy 
$$f^i(f^{-i}(a_{j-1})) =  a_{j-1} \leq f^i(y) < f^i(x) \leq a_j = f^i(f^{-i}(a_j)).$$
