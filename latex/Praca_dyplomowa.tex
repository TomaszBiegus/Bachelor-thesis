...
\item przestrzeń metryczna, metryka
\item orbita,
...
\item gęstość
\item napisac ze chaotycznosc odwzorowania rozumiem przez chaotycznosc odpowiedniego \item ukladu dynamicznego
...
\item[(h3)] \label{h_trzy} $h$ jest stałe na zwartym odcinku $[a,b] \subseteq \textrm{pr}_2(U_i)$ zawierającym punkt $y_0$ w swoim wnętrzu.
\end{enumerate}
...
Ponadto, ze względu na \ref{h_trzy},
...
W takim przypadku użyjemy lematu \ref{lemat_4_glownego_artykulu} aby dostać odwzorowanie $H = (f, h_x) \in \mathcal{F}$ takie, że $d_2(H, F) < \frac{\epsilon}{2}$ i dla $x \in \{x_0, f(x_0), \ldots, f^{r-1}(x_0)\}$ odwzorowania włóknowe $h_x$ są ściśle rosnące. (TODO uzasadnic, i odpowiedziec dlaczego na mocy lematu 4 mozemy dostac takie odwzorowanie z wloknami scisle rosnacymi) Ponieważ $y_0$ jest różnie od $0$ i $1$ dostajemy, że $\textrm{pr}_2(H^r(x_0, y_0))$ również jest różne od $0$ i $1$. (TODO uzasadnic) Następnie korzystając z przypadku 1 dostajemy odwzorowanie $G \in \mathcal{F}_{SO}^i$, dla którego zachodzi nierówność $d_2(G, H) < \frac{\epsilon}{2}$. (TODO uzasadnic) Wówczas $d_2(G, F) < \epsilon$. (TODO uzasadnic)
