...
TODO: granica gorna granica dolna - wystepuje przy entropii Bowena
...
Zbiór $\mathcal{F}$ jest niepustym, domkniętym podzbiorem ptrzestrzeni $C_{\triangle}(X \times I)$, 
\end{lemma}
...
% TODO
\textcolor{red}{W glownej pracy to byl lemat 3}
Niech $(X, \rho)$ będzie \textcolor{red}{separowalną przestrzenią metryczną drugiej kategorii (oryg. be a separable 2nd category metric space) TODO: Czy to tłymaczenie jest właściwe? Jeżeli tak to muszę dopisać definicje przestrzeni drugiej kategorii i separowalności}. Niech $f \in C(X)$ będzie topologicznie tranzytywnym odwzorowaniem, którego zbiór punktów okresowych jest gęsty w $X$. 
...
\begin{proof}
\textcolor{red}{TODO: wystarczy przepisać}
\end{proof}
...
Chcemy wykazać, że istnieje $F \in \mathcal{F}$ będące jednocześnie topologicznie tranzytywne i posiadające gęsty w $(X \times I)$ zbiór punktów okresowych. Z lematu \ref{F_jest_niepustym_domknietym_podzbiorem_trojkatnych} wiemy, że $\mathcal{F}$ jest niepustym, domkniętym podzbiorem ptrzestrzeni $C_{\triangle}(X \times I)$, która jak wynika z lematu \ref{ciagle_trojkatne_tworza_przestrzen_metryczna_zupelna} jest przestrzenią metryczną zupełną. 
