...
Zbiorem rezydualnym nazywamy dopełnienie zbioru mizernego.
\end{definition}
...
W przestrzeni zupełnej przekrój przeliczalnej mnogości zbiorów rezydualnych jest rezydualny. (A więc w szczególności niepusty).
\end{lemma}
...
\end{split}
\end{equation}
{\color{red}nowe: Szczegółowe uzasadnienie powyższych równości:
\begin{enumerate}
...
{\color{red}nowe: Szczegółowe uzasadnienie powyższej równości (Pytanie czy zastąpić powyższą równość przez poniższą, czy pozostawić obie w takiej formie jak jest teraz czyli najpierw zwięzła wersja a później, poniżej uzasadnienie?)
\begin{equation}
\begin{aligned}
...
{\color{red}nowe: Uzasadnienie:
W zasadzie chyba to widzę: $x_0$ wraca do $x_0$ bo odwzorowanie bazowe jest takie samo: $f$, po jednej iteracji cały odcinek $[a,b]$ przejdzie na $y_0$ +/- jakis epsilon (odleglosc miedzy odwzorowaniami) i w kolejnych iteracjach mozemy sie przesunac na osi Y o najwyzej (r+N+s-1) epsilonow, czyli znajdziemy taki epsilon, ze nie wyjdziemy poza odcinek [a,b].
Warto to rozpisać?
}
...
{\color{red}nowe: Uzasadnienie:
Powyższe nie jest dla mnie oczywiste. To wynika z jakiejś własności odwzorowań z $\mathcal{F}$? Autorzy użyli tutaj implicite jakiegoś twierdzenia o punkcie stałym?
}
...
W takim przypadku użyjemy lematu \ref{lemat_4_glownego_artykulu} aby dostać odwzorowanie $H = (f, h_x) \in \mathcal{F}$ takie, że $d_2(H, F) < \frac{\epsilon}{2}$ i dla $x \in \{x_0, f(x_0), \ldots, f^{r-1}(x_0)\}$ odwzorowania włóknowe $h_x$ są ściśle rosnące.

{\color{red}nowe: Chciałem to uzasadniać, ale po zerknięciu do lematu \ref{lemat_4_glownego_artykulu} zorientowałem się, że odwzorowania $h_x$ mogą być dowolne byle niemalejące, pozostawiajace krance niezmienione i dostatecznie bliskie oryginalnym. Warto umieścić tutaj jakiś komentarz uzasadniający? Może, uzasadnić dlaczego możemy znaleźć odwzorowanie ściśle rosnące dowolnie bliskie ustalonemu niemalejącemu?}

 
Ponieważ $y_0$ jest różnie od $0$ i $1$ dostajemy, że $\textrm{pr}_2(H^r(x_0, y_0))$ również jest różne od $0$ i $1$.

{\color{red}nowe: (Ponieważ $H = (f, h_x) \in \mathcal{F}$, to każde z odwzorowań $h_x$ pozostawia niezmienione punkty $0$ i $1$. Wprowadźmy oznaczenie $\textrm{pr}_2(H^r(x_0, y_0)) = (h_{f^{r-1}(x_0)} \circ \ldots \circ h_{f(x_0)} \circ h_{x_0})(y_0) = h_0(y_0)$. Odwzorowanie $h_0$ jako złożenie odwzorowań ściśle rosnących i pozostawiających punkty $0$ i $1$ niezmienione jest ściśle rosnące i pozostawia niezmienione punkty $0$ i $1$.  Ponieważ $y_0 > 0$ to oczywiście $h_0(y_0) > h_0(0) = 0$. Natomiast ze względu na fakt, że $y_0 < 1$, to zachodzi $h_0(y_0) < h_0(1) = 1$.)
}
