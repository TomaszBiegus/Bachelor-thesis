...
\begin{theorem}[O rozszerzaniu]
Niech $(X, \rho)$ będzie zwartą przestrzenią metryczną bez punktów izolowanych, oraz niech $f \in C(X)$ będzie odwzorowaniem chaotycznym w sensie Devaney'a. Wówczas odwzorowanie $f$ można rozszerzyć do odwzorowania $F \in C_{\triangle}(X \times I)$ (to znaczy tak, że $f$ jest odwzorowaniem bazowym dla $F$) w taki sposób, że $F$ jest również chaotyczne w sensie Devaney'a, $F$ ma taką samą entropię topologiczną jak $f$, oraz zbiory $X \times \{0\}$ i $X \times \{1\}$ są niezmiennicze ze względu na $F$.
\end{theorem}
